...
`)}`}]}}case "get_item_examples_from_registries":{let r=z.object({query:z.string(),registries:z.array(z.string())}).parse(n.params.arguments),i=await u(),s=await ba(r.registries,{query:r.query,config:i,useCache:!1});if(s.items.length===0)return {content:[{type:"text",text:a`No examples found for query "${r.query}".
...
`)}
              `}],isError:true};if(e instanceof a$1){let i=e.message;return e.suggestion&&(i+=`

\u{1F4A1} ${e.suggestion}`),e.context&&(i+=`

Context: ${JSON.stringify(e.context,null,2)}`),{content:[{type:"text",text:a`Error (${e.code}): ${i}`}],isError:true}}let r=e instanceof Error?e.message:String(e);return {content:[{type:"text",text:a`Error: ${r}`}],isError:true}}});export{k as a,j as b};//# sourceMappingURL=chunk-KAJFNXHN.js.map
//# sourceMappingURL=chunk-KAJFNXHN.js.map