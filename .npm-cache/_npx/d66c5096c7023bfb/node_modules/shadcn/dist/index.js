#!/usr/bin/env node
import {b as b$1,a as a$1}from'./chunk-KAJFNXHN.js';import {a,D,n,G,P,s,d,N,E as E$1,p,i,q,j,t,b,T as T$1,V,g,h,e,f,r,C,c as c$1,S,z as z$2,$,B as B$1,aa,Y as Y$1,W,Z,y,A,F,ba,_,H as H$1,k,U,o as o$1,u,L as L$1,M,m,I,l,J,v,x,w,K}from'./chunk-UHRSOZGS.js';export{Y as fetchTree,Z as getItemTargetPath,aa as getPreset,$ as getPresets,R as getRegistriesConfig,_ as getRegistriesIndex,O as getRegistry,W as getRegistryBaseColor,V as getRegistryBaseColors,U as getRegistryIcons,P as getRegistryItems,T as getRegistryStyles,S as getShadcnRegistryIndex,Q as resolveRegistryItems,X as resolveTree}from'./chunk-UHRSOZGS.js';import {c,o,n as n$1}from'./chunk-WKG3W635.js';import {f as f$1}from'./chunk-YPHLD64H.js';import'./chunk-6SDOGUEN.js';import*as E from'path';import E__default,{join}from'path';import {promises,existsSync}from'fs';import R from'fs-extra';import v$1 from'postcss';import ii from'postcss/lib/at-rule';import mt,{z as z$1}from'zod';import {execa}from'execa';import Fe from'prompts';import {SyntaxKind,Project,ScriptKind,VariableDeclarationKind}from'ts-morph';import et,{tmpdir}from'os';import {Command}from'commander';import fr from'deepmerge';import*as Y from'fs/promises';import Y__default from'fs/promises';import at from'dedent';import Rr from'open';import {diffLines}from'diff';import {StdioServerTransport}from'@modelcontextprotocol/sdk/server/stdio.js';import {randomBytes}from'crypto';import Dn from'fast-glob';async function Kt(e){let t$1={};if(!R.existsSync(e.cwd)||!R.existsSync(E__default.resolve(e.cwd,"package.json")))return t$1["1"]=true,{errors:t$1,projectInfo:null};let r=t("Preflight checks.",{silent:e.silent}).start();R.existsSync(E__default.resolve(e.cwd,"components.json"))&&!e.force&&(r?.fail(),s.break(),s.error(`A ${d.info("components.json")} file already exists at ${d.info(e.cwd)}.
...
`}}),e.append(n),e.insertBefore(n,v$1.comment({text:"---break---"}))),typeof i=="object")for(let[o,a]of Object.entries(i))if(o.startsWith("@")){let s=o.match(/@([a-zA-Z-]+)\s*(.*)/);if(s){let[,c,m]=s;Xt(n,c,m,a);}}else te(n,o,a);else if(typeof i=="string")try{let a=v$1.parse(`.temp{${i}}`).first;if(a&&a.nodes){let s=v$1.rule({selector:"temp",raws:{semicolon:!0,between:" ",before:`
  `}});a.nodes.forEach(c=>{if(c.type==="decl"){let m=c.clone();m.raws.before=`
    `,s.append(m);}}),s.nodes?.length&&n.append(s);}}catch(o){throw console.error("Error parsing at-rule content:",i,o),o}}function te(e,t,r){let i=e.nodes?.find(n=>n.type==="rule"&&n.selector===t);if(i||(i=v$1.rule({selector:t,raws:{semicolon:true,between:" ",before:`
  `}}),e.append(i)),typeof r=="object"){for(let[n,o]of Object.entries(r))if(n.startsWith("@")&&typeof o=="object"&&o!==null&&Object.keys(o).length===0){let a=n.match(/@([a-zA-Z-]+)\s*(.*)/);if(a){let[,s,c]=a,m=v$1.atRule({name:s,params:c,raws:{semicolon:true,before:`
    `}});i.append(m);}}else if(typeof o=="string"){let a=v$1.decl({prop:n,value:o,raws:{semicolon:true,before:`
    `}}),s=i.nodes?.find(c=>c.type==="decl"&&c.prop===n);s?s.replaceWith(a):i.append(a);}else if(typeof o=="object"){let a=n.startsWith("&")?t.replace(/^([^:]+)/,`$1${n.substring(1)}`):n;te(e,a,o);}}else if(typeof r=="string")try{let o=v$1.parse(`.temp{${r}}`).first;o&&o.nodes&&o.nodes.forEach(a=>{if(a.type==="decl"){let s=a.clone();s.raws.before=`
...
`}}),t.append(i),t.insertBefore(i,v$1.comment({text:"---break---"}))),r.forEach(({selector:n,apply:o})=>{i?.nodes?.find(s=>s.type==="rule"&&s.selector===n)||i?.append(v$1.rule({selector:n,nodes:[v$1.atRule({name:"apply",params:o,raws:{semicolon:true,before:`
    `}})],raws:{semicolon:true,between:" ",before:`
  `}}));});}}}function si(e){return {postcssPlugin:"update-css-vars",Once(t){let r=t.nodes.find(i=>i.type==="atrule"&&i.name==="layer"&&i.params==="base");r instanceof ii||(r=v$1.atRule({name:"layer",params:"base",nodes:[],raws:{semicolon:true,before:`
